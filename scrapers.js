...
const { countryFromNumber, normalizeNumber, safeText, BROWSER_HEADERS } = require('./utils');
...
async function fetchHTML(url, referer) {
  ...
  const res = await axios.get(url, {
    headers, timeout: TIMEOUT, maxRedirects: 5,
...
const COUNTRY_NAME_TO_CODE = {
  ...
  'canada': 'CA', 'germany': 'DE', 'france': 'FR', 'sweden': 'SE',
  'finland': 'FI', 'denmark': 'DK', 'netherlands': 'NL', 'spain': 'ES',
  'italy': 'IT', 'poland': 'PL', 'russia': 'RU', 'ukraine': 'UA',
  'romania': 'RO', 'czech republic': 'CZ', 'czechia': 'CZ', 'austria': 'AT',
  'switzerland': 'CH', 'ireland': 'IE', 'portugal': 'PT', 'belgium': 'BE',
  'norway': 'NO', 'estonia': 'EE', 'latvia': 'LV', 'lithuania': 'LT',
  'china': 'CN', 'hong kong': 'HK', 'taiwan': 'TW', 'japan': 'JP',
  'korea': 'KR', 'south korea': 'KR', 'india': 'IN', 'indonesia': 'ID',
  'philippines': 'PH', 'malaysia': 'MY', 'singapore': 'SG', 'thailand': 'TH',
  'vietnam': 'VN', 'australia': 'AU', 'new zealand': 'NZ', 'brazil': 'BR',
  'mexico': 'MX', 'argentina': 'AR', 'south africa': 'ZA', 'nigeria': 'NG',
  'egypt': 'EG', 'morocco': 'MA', 'saudi arabia': 'SA', 'uae': 'AE',
  'israel': 'IL', 'turkey': 'TR', 'pakistan': 'PK', 'bangladesh': 'BD',
  'hungary': 'HU', 'greece': 'GR', 'bulgaria': 'BG', 'slovakia': 'SK',
  'puerto rico': 'PR',
};
...
async function rso_listNumbers() {
  ...
  $('tr').each((_, tr) => {
    ...
    const url = href.startsWith('http') ? href : RSO_BASE + (href.startsWith('/') ? href : '/' + href);
...
async function rso_messages(number, url) {
  ...
  $('table').each((_, table) => {
    const $tbl = $(table);
    const heads = $tbl.find('thead th').map((_, th) => safeText($(th).text()).toLowerCase()).get();
    ...
    $tbl.find('tbody tr').each((_, tr) => {
      ...
      const message = safeText(msgHtml.replace(/<br\s*\/?>(\s*)/gi, '\n').replace(/<[^>]+>/g, ''));
      if (!message) return;
...
async function rc_listNumbers() {
  ...
  $('a.card-link').each((_, a) => {
    ...
    const country = (cc && /^[A-Za-z]{2}$/.test(cc)) ? cc.toUpperCase() : countryFromNumber(number);
    const url = href.startsWith('http') ? href : RC_BASE + (href.startsWith('/') ? href : '/' + href);
    if (!out.find(x => x.number === number)) {
...
async function rc_messages(number, url) {
  ...
  $('table tr').each((_, tr) => {
    ...
    if (message && message.toLowerCase() !== 'message' && from.toLowerCase() !== 'from') {
      msgs.push({ from, time, message });
    }
  });
  return msgs;
...
async function so_listNumbers() {
  ...
  $('.number-boxes-item').each((_, el) => {
    ...
    if (!out.find(x => x.number === number)) {
      out.push({ provider: 'smsonline', number, country, url: href, displayNumber: numText });
    }
  });
  return out;
}
...
async function fpn_listOne(countryPath, countryCode) {
  ...
  $('a.numbers-btn, a[href*="/receive-sms/"]').each((_, a) => {
    ...
    const number = normalizeNumber(m[1]);
    if (!number) return;
    const url = href.startsWith('http') ? href : FPN_BASE + (href.startsWith('/') ? href : '/' + href);
    if (!out.find(x => x.number === number)) {
...
const PROVIDERS = {
  'receivesmsonline': { name: 'receivesmsonline.net', listNumbers: rso_listNumbers, getMessages: rso_messages },
  'smsonline': { name: 'sms-online.co', listNumbers: so_listNumbers, getMessages: so_messages },
  'freephonenum': { name: 'freephonenum.com', listNumbers: fpn_listNumbers, getMessages: fpn_messages },
