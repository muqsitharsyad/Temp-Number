...
app.get('/api/numbers', async (req, res) => {
  ...
  results.forEach((r, idx) => {
    ...
    if (r.status === 'fulfilled') {
      r.value.forEach(n =>
        merged.push({ ...n, flag: flagFor(n.country), countryName: countryNameFor(n.country) })
      );
    } else {
      errors.push({ provider: id, error: String(r.reason && r.reason.message || r.reason) });
    }
  });
...
app.get('/api/messages', async (req, res) => {
  ...
  try {
    ...
    const msgs = await cache.wrap(key, 10_000, () => PROVIDERS[provider].getMessages(number, url));
